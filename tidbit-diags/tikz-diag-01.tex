...
\begin{scope}[rotate around z=58.5]
\filldraw[fill=blue!10, opacity=0.6] (0,-2,2) -- (0,-2,-2) --  (0,2,-2) -- (0,2,2) -- cycle;  
...
\path [-latex, blue, thick] (fO) edge (1.1,0,0) edge (0,1.1,0) edge (0,0,1.1) node [left, pos=0, anchor=east] {O};
\path [-latex, blue, rotate around ={20:(10,0)}, rotate around y=10, thick] (fN) edge +(1.1,0,0) edge +(0,1.1,0) edge +(0,0,1.1) node [above, anchor=north west] {N};
\draw [-latex, thin] (fO) -- (fD);
\path[-latex, green!70!black, bend left=35] (fO) edge  node[right,pos=0.75]{$|\mathbf{\uptheta}|$} (fD);
...
\fill[fill=green!50!yellow,opacity=0.1]  (fO)  --  node[left,pos=0.5]{$\mathbf{c}$} (ax)  --   node[below,pos=0.5]{$\mathbf{l}$}  (fD) to[out=108,in=-2] (fO) ; 
%\node at (6,0,8.5) {$l$};
...
\coordinate[label={$(3,0,2)$}] (B3) at (3,0,2);
\coordinate[label={$(0,3,2)$}] (C2) at (0,3,2);
...
from http://5.9.10.113/68788367/cylinder-node-with-opacity
...
from https://newbedev.com/how-to-create-curved-lines-in-tikz-that-go-through-specific-points

\begin{tikzpicture}[ scale=1.5, axis/.style={very thick, ->, >=stealth'},
important line/.style={thick}, dashed line/.style={dashed, thick}, every node/.style={color=black,} ]
...
from https://ctan.math.illinois.edu/graphics/pgf/contrib/tikz-layers/tikz-layers-doc.pdf


\begin{tikzpicture}[ node distance=-1mm, every node/.style={circle, text width=2cm, align=center, node font=\scriptsize}]%

\node[rectangle, fill=red, minimum height=1cm, opacity=.9, text width=6cm, align=left, outer sep=5mm] (main) at (0,0) {main layer};
...
from https://tex.stackexchange.com/questions/17204/drawing-polyhedra-using-tikz-with-semi-transparent-and-shading-effect
...
\begin{tikzpicture} [thick, z={(0, 1)}, x={(-0.866, -0.5)}, y={(-1,-1)},] %
\newcommand{\x}{2.5};  \newcommand{\y}{1.5};  \newcommand{\z}{.5};
\coordinate (A1) at (\x,\y,\z); \coordinate (B1) at (\x,\y,-\z);
\coordinate (A2) at (-\x,\y,\z); \coordinate (B2) at (-\x,\y,-\z);
\coordinate (A3) at (-\x,-\y,\z); \coordinate (B3) at (-\x,-\y,-\z);
\coordinate (A4) at (\x,-\y,\z); \coordinate (B4) at (\x,-\y,-\z);
\filldraw[gray!8,draw=gray!20] (A1) -- (A2) -- (A3) -- (A4) -- cycle ;
\fill[gray!32,draw=gray!40] (A1) -- (A2) -- (B2) -- (B1) -- cycle ;
\fill[gray!16,draw=gray!25] (A1) -- (A4) -- (B4) -- (B1) -- cycle ;
%\path[gray!60] (A1) edge (A4) edge (A2) edge (B1) ; 
\path[gray!50, dashed] (B3) edge (B4) edge (B2) edge (A3) ; 
%\draw[gray!40] (B4) -- (B1) -- (B2) -- (A2) -- (A3) -- (A4) -- cycle ;
\path[-latex, thick] ($(A1)!.5!(B3)$) edge[red!80!purple] ($(A1)!.5!(B4)$) edge[green!60!brown]  ($(A1)!.5!(B2)$) edge[blue!80!cyan] ($(A1)!.5!(A3)$) ;
\path[dotted, gray!50] ($(A1)!.5!(B3)$) edge[red!80!purple] ($(A2)!.5!(B3)$) edge[green!60!brown]  ($(A4)!.5!(B3)$) edge[blue!80!cyan] ($(B1)!.5!(B3)$) ;
\end{tikzpicture}
